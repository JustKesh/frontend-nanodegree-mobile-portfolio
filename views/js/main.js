...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      //removed the determineDX function and added switch(size)
      switch(size) {
        case "1":
          newWidth = 25;
              break;
      ...
      //Pulled the document selector out of the loop to reduce redundancy
...
function updatePositions() {
  ...
  window.performance.mark("mark_start_frame");
    ...
    //accesed the class in the DOM directly to improve performance
  var items = document.getElementsByClassName('mover');
    //moved repetitive code outside of the for loop, scrollTop/1250
    var phase = document.body.scrollTop / 1250;
    var position;
    
    //loop in reverse to evaluate .length only one and improve performance
  for (var i = items.length; i--;) {
    position = Math.sin(phase + (i % 5));
      //changed style.left to style.transform
    items[i].style.transform = 'translateX(' + 100 * position + 'px)';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 200; i++) {
    ...
    elem.style.width = "73.333px";
      //using transform in updatePositions() instead of basicLeft, a custom property, means this has to change to style.left
